...
export class UserService {
  ...
  myInfo(currentUser: number) {
    return this.userRepository.findOneBy({ id: currentUser })
  }
  ...
  async checkEmailType({ email }) {
    ...
    if (await this.userRepository.findOneBy({ username: email, active: 1 })) {
      return {
        hasAccount: true,
      }
    } else {
      if (await this.userRepository.findOneBy({ username: email, active: 0 })) {
        sendSignUp({
          receivers: [email],
  ...
  async sendTemporaryPassword({ email, password }) {
    const userRepositoryFindOneByUsernameAndActivateTrue =
      await this.userRepository.findOneBy({ username: email, active: 1 })
  ...
  async changePassword(currentUser, { oldPassword, newPassword }) {
    const currentUserInfo = await this.userRepository.findOneBy({
      id: currentUser,
    })
